...
one puts a prior over the ingredients given each cuisine. That means that it gives each 
ingredient a very small starting probability before looking at the data. This makes it so that
p(ingredient | cuisine) is never zero. Now, if you come across a cuisine that you've never 
...
The other small difference is that if you come across an ingredient that has never been seen 
in your training data, the algorithm will now just ignore it, rather than give it a probability 
of zero.
